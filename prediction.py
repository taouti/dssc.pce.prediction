...
def prepare_data(config: Config) -> Tuple[np.ndarray, np.ndarray, pd.Series, pd.Series]:
    ...
    return X_train_scaled, X_test_scaled, y_train, y_test, X.columns.tolist(), data['File']
...
if __name__ == "__main__":
    # Prepare Data
    config = Config()
    X_train, X_test, y_train, y_test, feature_names, dye_names = prepare_data(config)
    ...
    save_results_and_visualize(
        best_model,
        evaluation_metrics,
        pd.Series(y_test, index=dye_names),
        pd.Series(y_pred, index=dye_names),
        config,
        feature_names
    )